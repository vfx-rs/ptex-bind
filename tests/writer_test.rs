...
fn ptex_writer_f16() -> Result<()> {
    ...
    let alpha_channel = -1;
    let mesh_type = ptex::MeshType::Quad;
    let data_type = ptex::DataType::Float16;

    let filename = std::path::PathBuf::from("tests/tmp/ptex_writer_f16.ptx");
    if filename.exists() {
        fs::remove_file(&filename)?;
    }
    ...
    // Calculate the size for the f16 buffer used by write_face()
    let stride = 0;
    let size = get_buf_size(num_channels, &face_res);
    let mut buf: Vec<ptex::f16> = Vec::new();
    buf.resize(size, ptex::f16::from_f32(0.0));

    for i in 0..num_faces as usize {
        buf.fill(ptex::f16::from_f32(0.0));
        ...
        for v in 0..vres {
            for u in 0..ures {
                ...
                buf[idx] = ptex::f16::from_f32((u as f32) / ((ures - 1) as f32));
                buf[idx + 1] = ptex::f16::from_f32((v as f32) / ((vres - 1) as f32));
                buf[idx + 2] = ptex::f16::from_f32(color);
            }
        }
